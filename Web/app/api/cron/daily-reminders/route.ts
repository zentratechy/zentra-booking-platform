...
export async function GET(request: Request) {
  try {
    ...
    const currentHourString = `${String(currentHourUTC).padStart(2, '0')}:00`;
    
    console.log(`🕐 Current UTC hour: ${currentHourString} (${currentHourUTC}:00)`);

    // Get all businesses with reminders enabled
    const businessesQuery = query(collection(db, 'businesses'));
    const businessesSnapshot = await getDocs(businessesQuery);
    
    const allBusinesses = businessesSnapshot.docs
      .map(doc => ({ id: doc.id, ...doc.data() }))
      .filter((business: any) => business.dailyReminders?.enabled);

    // Filter businesses whose sendTime matches current hour
    const businesses = allBusinesses.filter((business: any) => {
      const sendTime = business.dailyReminders?.sendTime || '18:00'; // Default to 6 PM if not set
      const sendHour = sendTime.split(':')[0]; // Extract hour from "HH:00" format
      const matches = String(currentHourUTC) === sendHour;
      
      if (matches) {
        console.log(`✅ Business ${business.businessName || business.id} matches (sendTime: ${sendTime})`);
      }
      
      return matches;
    });

    console.log(`📊 Found ${businesses.length} businesses with reminders enabled for ${currentHourString} (out of ${allBusinesses.length} total enabled)`);
