...
export async function POST(request: Request) {
  try {
    ...
    let businessSettings: any = {
      loyaltyProgram: {} // Always initialize with empty object to prevent undefined errors
    };
    if (businessId) {
      try {
        ...
        if (businessDoc.exists()) {
          ...
          businessSettings = {
            ...
            loyaltyProgram: (businessData.loyaltyProgram && typeof businessData.loyaltyProgram === 'object' && businessData.loyaltyProgram !== null) ? businessData.loyaltyProgram : {} // Always ensure it's an object, never null or undefined
          };
        }
      } catch (error) {
        console.error('❌ Error fetching business settings:', error);
        // Continue with default settings (already has loyaltyProgram: {})
      }
    }
    ...
    if (type === 'booking_confirmation' && appointmentData) {
      ...
      try {
        // Ensure businessSettings has loyaltyProgram before calling template
        // Note: typeof null === 'object' in JavaScript, so we must explicitly check for null
        if (!businessSettings.loyaltyProgram || typeof businessSettings.loyaltyProgram !== 'object' || businessSettings.loyaltyProgram === null) {
          businessSettings.loyaltyProgram = {};
          console.log('⚠️ Set default loyaltyProgram to {}');
        }
        console.log('📝 BusinessSettings loyaltyProgram type:', typeof businessSettings.loyaltyProgram, 'value:', JSON.stringify(businessSettings.loyaltyProgram));
        ...
        console.error('❌ BusinessSettings at error:', JSON.stringify(businessSettings, null, 2));
        throw templateError;
      }
