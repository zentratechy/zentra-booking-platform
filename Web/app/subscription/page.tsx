...
export default function SubscriptionPage() {
  ...
  // Prevent any navigation away from this page and preserve expired parameter
  useEffect(() => {
    const currentPath = window.location.pathname;
    const currentSearch = window.location.search;
    
    // If we're on subscription page with expired=true, prevent navigation away
    if (currentPath === '/subscription' && currentSearch.includes('expired=true')) {
      // Block any attempts to navigate away
      const handleBeforeUnload = (e: BeforeUnloadEvent) => {
        // Only block if trying to navigate to my-bookings or other non-subscription pages
        // Allow navigation to dashboard after subscription
      };
      ...
      const checkNavigation = () => {
        if (window.location.pathname !== '/subscription' && 
          ...
          console.warn('[Subscription Page] Navigation detected to:', window.location.pathname);
  ...
  useEffect(() => {
    ...
    // especially for expired trial users
    ...
    // Preserve the expired parameter - if it was set to true, keep it
    // This prevents the URL from being changed by other components
    if (expired === 'true' && window.location.search !== '?expired=true') {
      // Only update if the URL doesn't already have the correct parameter
      const currentParams = new URLSearchParams(window.location.search);
      if (currentParams.get('expired') !== 'true') {
        currentParams.set('expired', 'true');
        router.replace(`/subscription?${currentParams.toString()}`, { scroll: false });
      }
    }
  ...
  // Debug: Log to console to help diagnose redirect issues
  useEffect(() => {
    console.log('[Subscription Page] Mounted, user:', user ? 'exists' : 'null', 'authLoading:', authLoading);
    return () => {
      console.log('[Subscription Page] Unmounting');
    };
  }, [user, authLoading]);
