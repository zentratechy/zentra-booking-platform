...
interface DragDropCalendarProps {
  ...
  onEditBlockTime?: (blockedTime: BlockedTime) => void;
  currency: string;
  businessHours: {
  ...
  serviceCategories?: Array<{name: string; color: string}>;
...
// Helper function to get color styles from hex color
const getColorStylesFromHex = (hexColor: string) => {
  ...
  // Use full color for border
  const borderColor = `rgb(${rgb.r}, ${rgb.g}, ${rgb.b})`;
  // Use darker version for text (80% darker)
  const textColor = `rgb(${Math.max(0, rgb.r - 51)}, ${Math.max(0, rgb.g - 51)}, ${Math.max(0, rgb.b - 51)})`;
...
const getServiceCategoryColor = (category: string, serviceCategories?: Array<{name: string; color: string}>) => {
  // If service categories are provided, use them
  if (serviceCategories && serviceCategories.length > 0) {
    const categoryData = serviceCategories.find(c => c.name === category);
    if (categoryData) {
      return getColorStylesFromHex(categoryData.color);
    }
  }
  
  // Fallback to default colors if no categories provided
...
const getStaffColor = (staffName: string, staffMembers?: Array<{id: string; name: string; color?: string}>) => {
  // If staff members with colors are provided, use the assigned color
  if (staffMembers && staffMembers.length > 0) {
    const staffMember = staffMembers.find(s => s.name === staffName);
    if (staffMember && staffMember.color) {
      return staffMember.color;
    }
  }
  
  // Fallback to default colors based on staff name for star highlighting
  const colors = [
    'text-purple-600',
    'text-pink-600',
    'text-indigo-600',
...
const DraggableAppointment: React.FC<{
  ...
  serviceCategories?: Array<{name: string; color: string}>;
  staffMembers?: Array<{id: string; name: string; color?: string}>;
}> = ({ appointment, currency, onEditAppointment, style, serviceCategories, staffMembers }) => {
  ...
  const categoryColorStyles = getServiceCategoryColor(appointment.serviceCategory || 'Other', serviceCategories);
  const isCustomColor = typeof categoryColorStyles === 'object' && categoryColorStyles !== null;
    ...
    <div
      ...
      className={`
        ...
        ${isCancelled 
          ...
          : isCustomColor 
            ? `cursor-pointer ${(isDragging || isTouchDragging) ? 'opacity-50 scale-95' : 'hover:shadow-md hover:scale-105'}` 
            : `${categoryColorStyles as string} cursor-pointer ${(isDragging || isTouchDragging) ? 'opacity-50 scale-95' : 'hover:shadow-md hover:scale-105'}`
        }
      `}
      style={{ 
        minHeight: '60px', 
        ...style,
        ...(isCustomColor && !isCancelled ? categoryColorStyles as React.CSSProperties : {}),
    ...
    >
      ...
      <div className="text-xs text-gray-500 flex items-center">
        ...
        <span className="flex items-center">
          {(() => {
            const staffColor = getStaffColor(appointment.staffName || 'Unknown Staff', staffMembers);
            const isHexColor = typeof staffColor === 'string' && staffColor.startsWith('#');
            return (
              <>
                <svg 
                  className={`mr-1 w-3 h-3 ${isHexColor ? '' : staffColor}`}
                  fill="currentColor" 
                  viewBox="0 0 20 20"
                  style={isHexColor ? { color: staffColor } : undefined}
                >
                  <path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z" />
                </svg>
                <span 
                  className={isHexColor ? '' : staffColor}
                  style={isHexColor ? { color: staffColor } : undefined}
                >
...
const DragDropCalendar: React.FC<DragDropCalendarProps> = ({
  ...
  const [selectedCategories, setSelectedCategories] = useState<Set<string>>(() => {
    ...
    if (typeof window !== 'undefined') {
      const saved = localStorage.getItem('calendar-category-filters');
  ...
  const handleCategoryFilterChange = (category: string, checked: boolean) => {
    ...
    // Save to localStorage
    if (typeof window !== 'undefined') {
      localStorage.setItem('calendar-category-filters', JSON.stringify([...newSelectedCategories]));
    }
  };
  ...
  React.useEffect(() => {
    if (appointments.length > 0) {
      // Get unique staff
      const uniqueStaffNames = getUniqueStaff();
      const savedStaff = typeof window !== 'undefined' 
        ? localStorage.getItem('calendar-staff-filters')
        : null;
      
      if (savedStaff) {
        // Clean up saved staff - only keep staff that actually exist
        try {
          const savedStaffArray = JSON.parse(savedStaff);
          const validStaff = savedStaffArray.filter((name: string) => uniqueStaffNames.includes(name));
          
          // Always validate and update if needed
          const currentSelected = Array.from(selectedStaff);
          const needsUpdate = validStaff.length !== currentSelected.length ||
                              !validStaff.every((name: string) => currentSelected.includes(name)) ||
                              !currentSelected.every((name: string) => validStaff.includes(name));
          
          if (needsUpdate) {
            if (validStaff.length > 0 && validStaff.length === uniqueStaffNames.length) {
              // All valid staff are selected, so select all (in case new staff was added)
              setSelectedStaff(new Set(uniqueStaffNames));
              if (typeof window !== 'undefined') {
                localStorage.setItem('calendar-staff-filters', JSON.stringify(uniqueStaffNames));
              }
            } else if (validStaff.length > 0) {
              // Some valid staff selected
              setSelectedStaff(new Set(validStaff));
              if (typeof window !== 'undefined') {
                localStorage.setItem('calendar-staff-filters', JSON.stringify(validStaff));
              }
            } else {
              // No valid staff, select all
              setSelectedStaff(new Set(uniqueStaffNames));
              if (typeof window !== 'undefined') {
                localStorage.setItem('calendar-staff-filters', JSON.stringify(uniqueStaffNames));
              }
      ...
      } else if (uniqueStaffNames.length > 0) {
        // No saved staff, select all by default
        setSelectedStaff(new Set(uniqueStaffNames));
        if (typeof window !== 'undefined') {
          localStorage.setItem('calendar-staff-filters', JSON.stringify(uniqueStaffNames));
        }
      ...
      appointments.forEach(apt => {
        if (apt.serviceCategory) {
          uniqueCats.add(apt.serviceCategory);
        } else if (apt.services && apt.services.length > 0) {
          apt.services.forEach((service: any) => {
            if (service.category) {
              uniqueCats.add(service.category);
            }
          });
        }
      });
      
      const savedCategories = typeof window !== 'undefined'
        ? localStorage.getItem('calendar-category-filters')
        : null;
      
      if (savedCategories) {
        // Clean up saved categories - only keep categories that actually exist
        try {
          const savedCategoriesArray = JSON.parse(savedCategories);
          const validCategories = savedCategoriesArray.filter((cat: string) => uniqueCats.has(cat));
          if (validCategories.length !== savedCategoriesArray.length || validCategories.length !== selectedCategories.size) {
            // Update if there are invalid categories or count mismatch
            if (validCategories.length > 0) {
              setSelectedCategories(new Set(validCategories));
              if (typeof window !== 'undefined') {
                localStorage.setItem('calendar-category-filters', JSON.stringify(validCategories));
              }
            } else {
              // If no valid categories, select all
              setSelectedCategories(uniqueCats);
              if (typeof window !== 'undefined') {
                localStorage.setItem('calendar-category-filters', JSON.stringify([...uniqueCats]));
              }
            }
          }
        } catch (e) {
      ...
      } else if (uniqueCats.size > 0) {
        // No saved categories, select all by default
        setSelectedCategories(uniqueCats);
        if (typeof window !== 'undefined') {
          localStorage.setItem('calendar-category-filters', JSON.stringify([...uniqueCats]));
        }
  ...
  const isBusinessHours = (timeSlot: string, date: Date) => {
    ...
    // If no open/close times set, consider it closed
  ...
  const getUniqueStaff = useCallback(() => {
  ...
  // Filter appointments by selected location, staff, categories, and exclude cancelled appointments
  const filteredAppointments = useMemo(() => {
    ...
    const uniqueStaffNames = getUniqueStaff();
    if (selectedStaff.size > 0 && selectedStaff.size < uniqueStaffNames.length) {
      filtered = filtered.filter(apt => selectedStaff.has(apt.staffName || 'Unknown Staff'));
    }
    
    // Filter by service category
    // Get all unique categories from appointments
    const allCategories = new Set<string>();
    appointments.forEach(apt => {
      if (apt.serviceCategory) {
        allCategories.add(apt.serviceCategory);
      } else if (apt.services && apt.services.length > 0) {
        apt.services.forEach((service: any) => {
          if (service.category) {
    ...
    if (selectedCategories.size > 0 && selectedCategories.size < allCategories.size) {
      filtered = filtered.filter(apt => {
        // Check if appointment has a category
        if (apt.serviceCategory) {
          return selectedCategories.has(apt.serviceCategory);
        }
        // Check if services array has any matching category
        if (apt.services && apt.services.length > 0) {
          return apt.services.some((service: any) => 
            service.category && selectedCategories.has(service.category)
          );
        }
        // If no category, exclude it when filters are active
        return false;
      });
    }
    ...
    return filtered;
  }, [appointments, selectedLocation, selectedStaff, selectedCategories, getUniqueStaff]);
  ...
  // Get blocked times for a specific date and time slot
  const getBlockedTimesForSlot = (date: Date, timeSlot: string) => {
    const dateString = date.toISOString().split('T')[0];
    
    return blockedTimes.filter(blocked => {
      // Handle Firestore Timestamp
      let startDate;
      if (blocked.startDate?.toDate) {
        startDate = blocked.startDate.toDate();
      } else if (blocked.startDate instanceof Date) {
      ...
      let endDate;
      if (blocked.endDate?.toDate) {
        endDate = blocked.endDate.toDate();
      } else if (blocked.endDate instanceof Date) {
        endDate = blocked.endDate;
      } else {
        endDate = new Date(blocked.endDate);
      }
      
      const blockedStartDateString = startDate.toISOString().split('T')[0];
      const blockedEndDateString = endDate.toISOString().split('T')[0];
      
      // Check if the date falls within the blocked time range
      if (dateString < blockedStartDateString || dateString > blockedEndDateString) {
        return false;
      }
      ...
      // If this is the start date, check if time is after start time
      if (dateString === blockedStartDateString) {
        const [blockedStartHours, blockedStartMinutes] = blocked.startTime.split(':').map(Number);
        const blockedStartTotalMinutes = blockedStartHours * 60 + blockedStartMinutes;
        if (slotTotalMinutes < blockedStartTotalMinutes) {
          return false;
        }
      }
      
      // If this is the end date, check if time is before end time
      if (dateString === blockedEndDateString) {
        const [blockedEndHours, blockedEndMinutes] = blocked.endTime.split(':').map(Number);
        const blockedEndTotalMinutes = blockedEndHours * 60 + blockedEndMinutes;
        if (slotTotalMinutes >= blockedEndTotalMinutes) {
          return false;
        }
      }
      
      // For dates in between start and end, show for all time slots
      // (already passed the date range check above)
      return true;
    });
  };
      ...
      <div className="bg-white rounded-lg shadow-lg overflow-hidden">
        ...
        <div className="bg-gray-50 px-6 py-4 border-b border-gray-200">
          <div className="flex items-center justify-between">
            ...
            <div className="mt-4 px-4 pb-4">
              <div className="flex flex-wrap gap-4 items-center">
                  ...
                  <div className="relative">
                    ...
                    >
                      ...
                      <span className="text-sm text-gray-600">
                        {selectedStaff.size === 0 || selectedStaff.size === uniqueStaff.length
                          ? 'All Staff'
                          : `${selectedStaff.size} Selected`}
                      </span>
                      <svg
                        className={`w-4 h-4 text-gray-500 transition-transform ${showStaffDropdown ? 'rotate-180' : ''}`}
                        fill="none"
                        stroke="currentColor"
                        viewBox="0 0 24 24"
                      >
                    ...
                    {showStaffDropdown && (
                      <div className="absolute top-full left-0 mt-2 w-64 bg-white rounded-lg border border-gray-200 shadow-lg z-50 max-h-96 overflow-y-auto">
                        <div className="p-2">
                          {uniqueStaff.map((staffName) => (
                            <label
                              key={staffName}
                              className="flex items-center space-x-2 cursor-pointer px-3 py-2 hover:bg-gray-50 rounded-md transition-colors"
                            >
                              <input
                                type="checkbox"
                              ...
                              />
                              {(() => {
                                const staffColor = getStaffColor(staffName, staffMembers);
                                const isHexColor = typeof staffColor === 'string' && staffColor.startsWith('#');
                                return (
                                  <>
                                    <svg 
                                      className={`w-4 h-4 ${isHexColor ? '' : staffColor}`}
                                      fill="currentColor" 
                                      viewBox="0 0 20 20"
                                      style={isHexColor ? { color: staffColor } : undefined}
                                    >
                                    ...
                                    <span 
                                      className={`text-sm text-gray-700 ${isHexColor ? '' : staffColor}`}
                                      style={isHexColor ? { color: staffColor } : undefined}
                                    >
                                      {staffName}
                                    </span>
                ...
                {(() => {
                  ...
                  const categories = Array.from(uniqueCats).sort();
            ...
            {(showStaffDropdown || showCategoryDropdown) && (
              <div
            ...
            {timeSlots.map((timeSlot, timeIndex) => (
              <div key={timeIndex} className="grid grid-cols-8 border-b border-gray-100">
                ...
                {weekDates.map((date, dayIndex) => {
                  ...
                  const slotBlockedTimes = getBlockedTimesForSlot(date, timeSlot);
                  return (
                    <DropZone
                      key={dayIndex}
                    ...
                    >
                      ...
                      {slotBlockedTimes.map((blocked) => {
                        // Handle Firestore Timestamp for date comparison
                        let startDate;
                        if (blocked.startDate?.toDate) {
                          startDate = blocked.startDate.toDate();
                        } else if (blocked.startDate instanceof Date) {
                        ...
                        // Determine the effective start and end times for this date
                        let effectiveStartTime = blocked.startTime;
                        let effectiveEndTime = blocked.endTime;
                        
                        // If this is not the start date, start from beginning of day
                        if (dateString !== blockedStartDateString) {
                          effectiveStartTime = '00:00';
                        }
                        
                        // If this is not the end date, end at end of day
                        if (dateString !== blockedEndDateString) {
                          effectiveEndTime = '23:59';
                        }
                        
                        // Calculate duration in minutes for this day
                        ...
                        const blockedSlots = Math.ceil(duration / 15);
                        const slotHeight = 60;
                        const blockedHeight = Math.max(blockedSlots * slotHeight - 16, 60);
                        
                        // Get staff member name
                        const staffMember = staffMembers.find(s => s.id === blocked.staffId);
                        const staffName = staffMember?.name || 'Unknown Staff';
                        
                        // Check if this is the starting slot for this blocked time on this date
                        const currentTimeIndex = timeSlots.indexOf(timeSlot);
                        let blockedTimeIndex = timeSlots.indexOf(effectiveStartTime);
                        
                        if (blockedTimeIndex === -1) {
                          const [blockedHours, blockedMins] = effectiveStartTime.split(':').map(Number);
                          const blockedTotalMinutes = blockedHours * 60 + blockedMins;
                          ...
                          for (let i = 0; i < timeSlots.length; i++) {
                            const [slotHours, slotMinutes] = timeSlots[i].split(':').map(Number);
                            const slotTotalMinutes = slotHours * 60 + slotMinutes;
                            const difference = Math.abs(slotTotalMinutes - blockedTotalMinutes);
                          ...
                          if (closestIndex !== -1) {
                            blockedTimeIndex = closestIndex;
                          }
                        }
                        
                        const isStartingSlot = currentTimeIndex === blockedTimeIndex;
                        if (!isStartingSlot) return null;
                        
                        return (
                          <div
                            key={`${blocked.id}-${dateString}`}
                            onClick={(e) => {
                              e.stopPropagation();
                              if (onEditBlockTime) {
                                onEditBlockTime(blocked);
                              }
                            }}
                            style={{
                              ...
                              cursor: onEditBlockTime ? 'pointer' : 'default',
                            }}
                            className="blocked-time hover:bg-red-100 transition-colors"
                          >
                            <div className="font-medium text-xs text-red-800 truncate">
                              🚫 {blocked.title}
                            </div>
